...
using Square.Default.Cards;
using Square.Test.Unit.MockServer;

namespace Square.Test.Unit.MockServer.Default.Cards;
...
public class GetTest : BaseMockServerTest
{
    ...
    public async Task MockServerTest()
    {
        ...
        var response = await Client.Default.Cards.GetAsync(
            new GetCardsRequest { CardId = "card_id" }
        );
        Assert.That(
            response,
            Is.EqualTo(JsonUtils.Deserialize<GetCardResponse>(mockResponse)).UsingDefaults()
        );
    }
}
