...
namespace Square.Legacy.Apis
{
    ...
    internal class CatalogApi : BaseApi, ICatalogApi
    {
        ...
        /// </summary>
        internal CatalogApi(GlobalConfiguration globalConfiguration)
                            ...
                            _parameters
                                ...
                                .Form(_form =>
                                    _form
                                        .EncodingHeader(
                                            "Content-Type",
                                            string.IsNullOrEmpty(imageFile.ContentType)
                                                ? "image/jpeg"
                                                : imageFile.ContentType
                                        )
                                        .Setup("image_file", imageFile)
                                )
                        )
                )
                .ResponseHandler(_responseHandler =>
        ...
        public Models.CatalogInfoResponse CatalogInfo() => CoreHelper.RunTask(CatalogInfoAsync());
        ...
        public async Task<Models.CatalogInfoResponse> CatalogInfoAsync(
            ...
            await CreateApiCall<Models.CatalogInfoResponse>()
                .RequestBuilder(_requestBuilder =>
                    _requestBuilder.Setup(HttpMethod.Get, "/v2/catalog/info").WithAuth("global")
                )
                .ResponseHandler(_responseHandler =>
                    _responseHandler.ContextAdder(
                        (_result, _context) => _result.ContextSetter(_context)
                    ...
                    _requestBuilder
                        ...
                        .Parameters(_parameters =>
                            _parameters.Template(_template =>
                                _template.Setup("object_id", objectId)
                            )
                        )
                )
                .ResponseHandler(_responseHandler =>
