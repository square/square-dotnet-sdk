...
namespace Square.Legacy.Tests.Flows
{
    [TestFixture]
    public class CustomersApiFlowTest : ApiTestBase
    {
        ...
        {
            var response = await customAttributesApi.DeleteCustomerCustomAttributeDefinitionAsync(
        ...
        private async Task<CustomAttributeDefinition> CreateCustomerCustomAttributeDefinition()
        {
            var body = new CreateCustomerCustomAttributeDefinitionRequest(
                new CustomAttributeDefinition(
                    ...
                    schema: JsonObject.FromJsonString(
            ...
            var response = await customAttributesApi.CreateCustomerCustomAttributeDefinitionAsync(
                body
            );
            ...
            Assert.AreEqual(
                "The customer's favorite drink",
                response.CustomAttributeDefinition.Description
            );
            Assert.AreEqual(
                "VISIBILITY_READ_WRITE_VALUES",
                response.CustomAttributeDefinition.Visibility
            );
            Assert.AreEqual(1, response.CustomAttributeDefinition.Version);
            Assert.AreEqual(
                "{\"$ref\":\"https://developer-production-s.squarecdn.com/schemas/v1/common.json#squareup.common.String\"}",
                response.CustomAttributeDefinition.Schema.ToString()
            );
        ...
        private async Task<CustomAttribute> CreateCustomerCustomAttribute(string customerId)
        {
            var body = new UpsertCustomerCustomAttributeRequest(
                new CustomAttribute(mValue: JsonValue.FromString("Double-shot breve"))
            );
            var response = await customAttributesApi.UpsertCustomerCustomAttributeAsync(
                customerId,
                CUSTOM_ATTRIBUTE_KEY,
                body
            );
